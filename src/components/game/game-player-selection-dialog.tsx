...
interface GamePlayerSelectionDialogProps {
  onConfirm: () => void
...
function GamePlayerSelectionDialog(props: GamePlayerSelectionDialogProps) {
  return (
    <Dialog onOpenChange={props.onOpenChange} open={props.open}>
      <DialogContent className="w-[calc(100svw-2rem)] max-w-[calc(100svw-2rem)] rounded-2xl border-4 border-game-ink bg-white p-6 text-game-ink shadow-[0_8px_0_0_#16171d] sm:max-w-md">
        <DialogHeader className="pr-10">
          <DialogTitle className="text-2xl font-black tracking-[-0.04em]">Confirmer la sélection</DialogTitle>
        </DialogHeader>
        <p className="mt-3 text-sm font-bold leading-6 text-game-ink/70">
          {props.player ? `${props.player.name} commencera son tour.` : 'Ce joueur commencera son tour.'}
        </p>
...
export { GamePlayerSelectionDialog }
